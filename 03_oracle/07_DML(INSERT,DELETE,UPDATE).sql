...
2) INSERT INTO 테이블명 (컬럼명1, 컬럼명2, 컬럼명3)
        ...
        제시되지 않는 컬럼에 대한 기본값은 기본적으로 NULL이 저장되고,
        NULL값이 아닌 다른 값으로 저장하고자 할 때는 가장 기본값 션으로 설득된다
        -> 제시 되지 않은 컬럼에 NOT NULL 제약조건이 있다면, 반드시 값을 직접 제시해야 함
            또는 기본값 옵션을 추가해야 함
*/
INSERT INTO EMPLOYEE (EMP_ID, EMP_NO, EMP_NAME, EMAIL, JOB_CODE)
    VALUES (901, '000000-0000000', '오라클', 'oracle@co.kr' ,'J7');
    
--------------------------------------------------------------------------------
/*
    3) INSERT INTO 테이블명 (서브쿼리);
    -> VALUES로 값을 직접 명시하는 대신
    서브쿼리로 조회된 결과값을 통재로 INSERT 하는 방법
    (여러 행 추가하는 방식)
*/
...
-- 위의 쿼리문 결과를 가지고 EMP01테이블에 데이터 추가
INSERT INTO EMP01
(SELECT EMP_ID, EMP_NAME, DEPT_TITLE
FROM EMPLOYEE
LEFT JOIN DEPARTMENT ON DEPT_CODE = DEPT_ID);
